...
const communities = [
  ...
  { name: "Job Preparations" },
  { name: "LinkedIn & AI Prompting" },
];
...
const applicationEmail = "careercatalystcommunity@gmail.com";
const mailSubject = "Internship Application - Career Catalyst Community";
  ...
  <div className="min-h-screen flex flex-col bg-background">
    ...
    <main className="flex-1 py-20 md:py-28">
      <div className="w-full max-w-4xl mx-auto px-4 md:px-8 lg:px-12">
        <h2 className="text-4xl md:text-5xl font-bold text-brand-oxford mb-8 text-center">Apply for Internship</h2>
        <div className="mb-12">
          ...
          <ul className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {communities.map((c) => (
              <li key={c.name} className="bg-white rounded-xl p-4 flex items-center justify-between border border-primary/20">
                ...
                >
                  <a href={`mailto:${applicationEmail}?subject=${encodeURIComponent(mailSubject)}`}>Apply</a>
                </Button>
              </li>
            ))}
          </ul>
